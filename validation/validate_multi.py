...
def main():
    ...
    sim_uris = {p["body"]["tag"]["address"].get("namespaceUri") for p in sim}
    kep_uris = {p["body"]["tag"]["address"].get("namespaceUri") for p in kep}
    sim_tags = {p["body"]["tag"]["address"].get("nodeId") for p in sim}
    kep_tags = {p["body"]["tag"]["address"].get("nodeId") for p in kep}
    ...
    check("sim1 tags are Sine*", any((t or "").startswith("Sine") for t in sim_tags), f"{sorted(t for t in sim_tags if t)}")
    check("kep1 tags are Live*", any("Live" in (t or "") for t in kep_tags), f"{sorted(t for t in kep_tags if t)}")

    # derive component name from a sim1 topic: southbound/<site>/<comp>/sim1/<tag>
    comp = None
    for t, p in msgs:
        parts = t.split("/")
        if len(parts) >= 5 and parts[3] == "sim1":
            ...
            break
    if comp:
        # on-demand read on EACH instance's own topic resolves only that server's tag
        rs = request(c, f"southbound/{comp}/sim1/read", {"tags": [{"namespaceUri": SIM_NS, "tagId": "Counter"}]})
        sids = {e["tag"]["address"].get("nodeId"): e.get("quality") for e in (rs.get("body", {}).get("reads", []) if rs else [])}
        check("sim1 read routes to sim", sids.get("Counter") == "GOOD", f"{sids}")

        rk = request(c, f"southbound/{comp}/kep1/read", {"tags": [{"namespaceUri": KEP_NS, "tagId": "GGCommonsTest.Device1.Int32"}]})
        kids = {e["tag"]["address"].get("nodeId"): e.get("quality") for e in (rk.get("body", {}).get("reads", []) if rk else [])}
