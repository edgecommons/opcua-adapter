 ...
 * {@code { "namespace": <int>, "match": "<regex>", "topic": "<optional>", "samplingRateMs": <num>,
 * "queueSize": <int>, "deadband": {...} }}. {@code match} is a regex applied to the node's
 * identifier, browse name, or display name.
 */
public class TagSpec {
    ...
    private TagSpec(int namespace, String match, String topic, double samplingRateMs, int queueSize, DeadbandSpec deadband) {
    ...
    public static TagSpec fromJson(JsonObject o, double defaultSamplingMs, int defaultQueueSize) {
        ...
        return new TagSpec(ns, match, topic, sampling, queue, deadband);
    }
}
