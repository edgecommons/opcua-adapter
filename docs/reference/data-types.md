...
| `<T>[]` (array of any row above) | `<T>Array` (scalar code + 20, e.g. WordArray = 25) | array — each element by its row's rule | array — coerced element-wise; length must match the signal's array dimension |
...
An array signal (OPC UA `ValueRank` ≥ 1) is carried as a **JSON array**, each element rendered by its
...
as `value`; elements are coerced to the node's element type and the array length must equal the signal's
declared dimension.
...
- **Number precision.** Integers are emitted as JSON numbers across the full 64-bit range. Consumers
  ...
  `|value| > 2^53`; parse such signals as big integers if exactness matters.
