...
library inbox and delegates dispatch to the pure `CommandRouter`, which routes on the `instance`
selector through the `DeviceSession` seam.
...
|----|----------|-----------|
| D-OPCUA-1 | **Component-level command inbox, instance routing by the `instance` body field.** The library `CommandInbox` is one-per-component; this adapter is multi-instance. Verbs register once; each request carries an optional `instance` selector (required only when >1 instance is connected). | Per-instance inboxes are not a shipped library facade. Routing on the body field is the sanctioned multi-instance pattern (D-EIP-13 / D-U28). |
...
| D-OPCUA-3 | **`sb/browse` returns hierarchical reference trees** (paged, `depth` 0..4, `maxRefs` 1..2000, `truncated` flag). | The reference implementation for address-space discovery; bounded to protect the server and the bus. |
...
| D-OPCUA-8 | **`southbound_health` carries the exact SOUTHBOUND.md §5 measure set:** `connectionState`, `publishLatencyMs`, `pollLatencyMs`, `readErrors`, `staleSignals`, plus the §5-optional `reconnects` and `writeErrors`. `publishLatencyMs` is the last `data()` publish round-trip; `pollLatencyMs` is the last explicit-read round-trip (`repoll`/`sb/read`); `staleSignals` counts subscribed signals with no update past `healthThresholds.staleSignalSecs` (default 30), tracked per-signal in `HealthState` and seeded at subscription time so a signal that never ticks is visible. | Fleet dashboards read every adapter's health the same way; the previous 3-measure set (connectionState/readErrors/writeErrors) was below the canonical contract. |
